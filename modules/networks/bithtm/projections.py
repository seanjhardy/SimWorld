...
class DenseProjection:
    def __init__(self, input_dim, output_dim,
            permanence_mean=0.0, permanence_std=0.01,
            permanence_threshold=0, permanence_increment=0.02, permanence_decrement=0.03):
        ...
        self.permanence_std = permanence_std
        self.permanence_mean = permanence_mean
        
        self.permanence = np.zeros((output_dim, input_dim), dtype=np.float16)
        for x in range(output_dim):
            self.permanence[x] = np.random.randn(input_dim).astype(np.float16) * permanence_std + permanence_mean
    
    def invert(self, output_activation):
        overlaps = np.sum(self.permanence[output_activation] > self.permanence_threshold, axis=0)
        return overlaps

    def process(self, input_activation):
        overlaps = np.sum(self.permanence.T[input_activation] > self.permanence_threshold, axis=0)
        return overlaps
    
    def randomise(self, neurons):
        self.permanence[:, neurons] = np.random.randn(self.input_dim, len(neurons)).astype(np.float16) * self.permanence_std + self.permanence_mean
        
    def update(self, input_activation, learning_output, permanence_incr=None, permanence_decr=None):
        permanence_increment = permanence_incr or self.permanence_increment
        permanence_decrement = permanence_decr or self.permanence_decrement
        
        self.permanence[learning_output] += input_activation * (permanence_increment + permanence_decrement) - permanence_decrement
        
        selected = self.permanence[learning_output]
        self.permanence[learning_output] = np.clip(selected, -1, 1)


class SparseProjection:
    def __init__(
        self, input_dim, output_dim=0,
        output_growth_exponential=False, edge_growth_exponential=False
    ):
        self.input_dim = input_dim
        self.output_dim = output_dim
    ...
    def update(
        ...
        permanence_initial=0.21, active_edge_permanence_change=0.3, inactive_edge_permanence_change=0.1
    ):
...
class PredictiveProjection:
    ...
    def __init__(
        self, output_dim,
        permanence_initial=0.21, permanence_threshold=0.2, permanence_increment=0.3, permanence_decrement=0.1, permanence_punishment=0.02,
        segment_activation_threshold=10, segment_matching_threshold=10, segment_sampling_synapses=20,
        segment_bundle_growth_exponential=False
    ):
