...
from modules.networks.vision.scae.models.primary import PrimaryImageEncoder, TemplateBasedImageDecoder
from modules.networks.vision.scae.models.attention import SetTransformer
from modules.networks.vision.scae.models.capsule import CapsuleLayer, CapsuleLikelihood, sparsity_loss
from modules.networks.vision.scae.models.utils import flat_reduce, batch_flatten
from modules.networks.vision.scae.models.probe import ClassificationProbe
from modules.networks.vision.scae.models.prob import MixtureDistribution
...
class ImageAutoencoder(nn.Module):
    ...
    def forward(self, img, label, device, labeled=None):
        ...
        mixing_logits, loc = rec.pdf
        print(mixing_logits.shape, loc.shape, img.shape, mixing_log_prob.shape)
        res.rec_ll_per_pixel = MixtureDistribution.log_prob(*rec.pdf, img, mixing_log_prob)
        res.rec_mode = MixtureDistribution.mode(*rec.pdf, mixing_log_prob)
        
        res.new_pdf = MixtureDistribution.mode(*rec.pdf, mixing_log_prob)
        ...
        if label is not None:
            ...
            res.prior_cls_xe, res.prior_cls_acc = self._classification(
                ...
                label,
                labeled)
        if "prior_cls_acc" in res and "posterior_cls_acc" in res:
