...
from modules.networks.vision.scae.setmodules import *
...
class PCAE(nn.Module):
    def __init__(self, config, num_capsules=24, template_size=11, num_templates=24,num_feature_maps=24):
        ...
        self.capsules = nn.Sequential(nn.Conv2d(3,128,(1,3),stride=2),
                                      nn.ReLU(),
                                      nn.Conv2d(128,128,(1,3),stride=2),
                                      nn.ReLU(),
                                      nn.Conv2d(128,128,(1,3),stride=1),
                                      nn.ReLU(),
                                      nn.Conv2d(128,128,(1,3),stride=1),
                                      nn.ReLU(),
    ...
    def forward(self, x, device, mode='train'):
        ...
        gaussians = multiplier*((((detach_x - transformed_templates)**2)*power_multiply).exp()) #(B,M,C,W,H)
...
class SCAE(nn.Module):
    def __init__(self, config=None, device="cuda"):
        ...
        self.device = device
        self.dataParallel = nn.DataParallel(self)
        self.optimizer = optim.RMSprop(self.model.parameters(), lr=config.lr,
                                  momentum=0.9, eps=(1/(10*config.batch_size)**2) )
        self.loss = SCAE_LOSS()
        self.to(device)
        self.K = 24
        self.C = 10
    ...
    def forward(self, x, mode="test"):
        image_likelihood, input_ocae, x_m, d_m = self.model.pcae(x, self.device,mode)
        part_likelihood, a_k, a_kn, gaussian = self.model.ocae(input_ocae,x_m,d_m,self.device,mode)
    ...
    def backward(self, x):
        ...
        out = self.forward(x, mode="train")
        k_c = torch.tensor(float(self.K/self.C)).to(self.device)
        b_c = torch.tensor(float(b/self.C)).to(self.device)
        loss = self.loss(out, b_c=b_c, kc=k_c)
        loss.mean().backward()
        torch.nn.utils.clip_grad_norm(self.parameters(), 5)
        self.optimizer.step()

        latent = np.zeros(100)
        img_lik, part_lik, a_k, a_kn, gaussian = out

        print(img_lik.shape, part_lik.shape, a_k.shape, gaussian.shape)
