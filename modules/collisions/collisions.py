...
def solve_collisions(grid, character):
    ...
    check_collision[blocks_per_grid, threads_per_block](d_grid, character.body[0].x, character.body[0].y, character.size, d_out)
    d_out_copy = d_out.copy_to_host()

    if abs(d_out_copy[0]) > 0.05 or abs(d_out_copy[1]) > 0.05:
        character.collision_force = abs(d_out_copy[0]) + abs(d_out_copy[1])
    character.body[0].x += d_out_copy[0] * 1.1
    character.body[0].y += d_out_copy[1] * 1.1
